...
export default function DynamicFormContent() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // ✅ Helper برای آپلود فایل به Vercel Blob
      const uploadFile = async (file: File): Promise<string> => {
        const uploadData = new FormData();
        uploadData.append("file", file);
        const res = await fetch("/api/upload", {
      ...
      if (galleryInput?.files?.length) {
        ...
        finalData.Gallery_Image_File_Names = galleryUrls.join(",");
      }
  ...
  const renderField = (field: Field) => {
    ...
    // ✅ برای فیلدهای فایل: دیگه مقدار اسم فایل رو توی فرم ذخیره نمی‌کنیم
    if (field.type === "imageUpload" || field.type === "file") {
      return (
        <input
          type="file"
